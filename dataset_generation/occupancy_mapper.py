...
from multimodal_challenge.paths import OBJECT_INIT_DIRECTORY, OCCUPANCY_MAPS_DIRECTORY
from multimodal_challenge.util import NUM_LAYOUTS, get_object_init_commands
...
class OccupancyMapper(FloorplanController):
    ...
    def create(self) -> None:
        ...
        scene_images_dir = str(Path("../doc/images/scenes").resolve())
        ...
        for f in OBJECT_INIT_DIRECTORY.iterdir():
            ...
            for layout in range(NUM_LAYOUTS):
                ...
                TDWUtils.save_images(images=images, filename=f"{scene}_{layout}", output_directory=scene_images_dir,
                                     append_pass=False)
                ...
                np.save(str(OCCUPANCY_MAPS_DIRECTORY.joinpath(f.name[:-5]).resolve()), occupancy_map)
